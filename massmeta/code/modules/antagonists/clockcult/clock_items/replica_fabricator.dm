...
/obj/item/clockwork/replica_fabricator
	...
	worn_icon_state = "replica_fabricator"
...
/obj/item/clockwork/replica_fabricator/examine(mob/living/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		if(uses_power)
			. += {"<span class='alloy'>It can consume floor tiles, rods, metal, and plasteel for power at rates of <b>2:[display_energy(POWER_ROD)]</b>, <b>1:[display_energy(POWER_ROD)]</b>, <b>1:[display_energy(POWER_METAL)]</b>, \
			and <b>1:[display_energy(POWER_PLASTEEL)]</b>, respectively.</span>\n
			<span class='alloy'>It can also consume brass sheets for power at a rate of <b>1:[display_energy(POWER_FLOOR)]</b>.</span>\n
			<span class='alloy'>Use it in-hand to produce <b>5</b> brass sheets at a cost of <b>[display_energy(POWER_WALL_TOTAL)]</b> power.</span>\n
			<span class='alloy'>It has access to <b>[display_energy(get_clockwork_power())]</b> of power.</span>"}

/obj/item/clockwork/replica_fabricator/attack_self(mob/living/user)
	if(is_servant_of_ratvar(user))
		if(uses_power)
			if(!get_clockwork_power(POWER_WALL_TOTAL))
				to_chat(user, "<span class='warning'>[src] requires <b>[display_energy(POWER_WALL_TOTAL)]</b> of power to produce brass sheets!</span>")
				return
			adjust_clockwork_power(-POWER_WALL_TOTAL)
		playsound(src, 'sound/items/deconstruct.ogg', 50, TRUE)
		new/obj/item/stack/tile/brass(user.loc, 5)
		to_chat(user, "<span class='brass'>You use [get_clockwork_power() ? "some":"all"] of [src]'s power to produce <b>5</b> brass sheets. It now has access to <b>[display_energy(get_clockwork_power())]</b> of power.</span>")
	...
	if(!get_clockwork_power(fabrication_values["power_cost"]))
		if(get_clockwork_power() - fabrication_values["power_cost"] < 0)
			if(!silent)
				var/atom/A = fabrication_values["new_obj_type"]
				if(A)
					to_chat(user, "<span class='warning'>You need <b>[display_energy(fabrication_values["power_cost"])]</b> power to fabricate \a [initial(A.name)] from [target]!</span>")
		return FALSE
	return TRUE
...
/obj/item/clockwork/replica_fabricator/proc/fabricator_repair_checks(list/repair_values, atom/target, mob/user, silent) //Exists entirely to avoid an otherwise unreadable series of checks.
	...
	if(!GLOB.ratvar_awakens && !get_clockwork_power(repair_values["power_required"]))
		if(!silent)
			to_chat(user, "<span class='warning'>You need at least <b>[display_energy(repair_values["power_required"])]</b> power to start repairin[target == user ? "g yourself" : "g [target]"], and at least \
			<b>[display_energy(repair_values["amount_to_heal"]*MIN_CLOCKCULT_POWER, MIN_CLOCKCULT_POWER)]</b> to fully repair [target == user ? "yourself" : "[target.p_them()]"]!</span>")
		return FALSE
	return TRUE
