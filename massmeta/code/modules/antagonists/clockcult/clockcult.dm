...
/datum/antagonist/clockcult/apply_innate_effects(mob/living/mob_override)
	...
	if(issilicon(current))
		...
		if(iscyborg(S))
			...
			var/list/AI_frame = list(mutable_appearance('massmeta/icons/mob/clockwork_mobs.dmi', "aiframe")) //make the AI's cool frame
			for(var/d in GLOB.cardinals)
				AI_frame += image('massmeta/icons/mob/clockwork_mobs.dmi', A, "eye[rand(1, 10)]", dir = d) //the eyes are randomly fast or slow
