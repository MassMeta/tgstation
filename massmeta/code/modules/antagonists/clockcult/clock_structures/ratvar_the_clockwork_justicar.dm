...
/obj/structure/destructible/clockwork/massive/ratvar/Initialize()
	...
	var/mutable_appearance/alert_overlay = mutable_appearance('massmeta/icons/effects/clockwork_effects.dmi', "ratvar_alert")
...
/obj/structure/destructible/clockwork/massive/ratvar/process()
	if(clashing) //I'm a bit occupied right now, thanks
		return
	for(var/turf/T in circle_range(src, convert_range))
		if(!istype(T))
			continue
		T.ratvar_act()
	for(var/turf/T in circle_view(src, round(convert_range * 0.5)))
		if(!istype(T))
			continue
		T.ratvar_act(TRUE)
	...
	if(!prey)
		...
		if((!istype(prey, /obj/narsie) && prob(10) && LAZYLEN(meals) > 1) || prey.z != z || !(prey in meals))
