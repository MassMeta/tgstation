...
/proc/generate_all_scripture()
	if(!GLOB.all_scripture.len)
		for(var/V in sort_list(list(subtypesof(/datum/clockwork_scripture), /proc/cmp_clockscripture_priority)))
...
/proc/can_recite_scripture(mob/living/L, can_potentially)
	return (is_servant_of_ratvar(L) && (can_potentially || (L.stat == CONSCIOUS && L.can_speak())) && (GLOB.ratvar_awakens || (ishuman(L) || issilicon(L))))
