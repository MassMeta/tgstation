...
/datum/action/innate/darkspawn/divulge/Activate()
	...
	notify_ghosts("Darkspawn [user.real_name] has begun divulging at [get_area(user)]! ", source = user, ghost_sound = 'massmeta/sound/magic/devour_will_victim.ogg', alert_overlay = alert_overlay, action = NOTIFY_ORBIT)
	...
	for(var/stage in 1 to 3)
		switch(stage)
			if(1)
				...
				playsound(user, 'massmeta/sound/magic/divulge_01.ogg', 30, 0)
			if(2)
				...
				playsound(user, 'massmeta/sound/magic/divulge_02.ogg', 40, 0)
			if(3)
				...
				playsound(user, 'massmeta/sound/magic/divulge_03.ogg', 50, 0)
		if(!do_after(user, 15 SECONDS, user))
			...
			in_use = FALSE
			return
	playsound(user, 'massmeta/sound/magic/divulge_ending.ogg', 50, 0)
	...
	playsound(user, 'massmeta/sound/magic/divulge_end.ogg', 50, 0)
