...
/datum/action/innate/darkspawn/pass/Activate()
	...
	if(!darkspawn.upgrades["twin_tendrils"] || hands_free.len < 2)
		...
		playsound(owner, 'massmeta/sound/magic/pass_create.ogg', 50, 1)
		...
		playsound(owner, 'massmeta/sound/magic/pass_create.ogg', 50, TRUE)
		addtimer(CALLBACK(GLOBAL_PROC, .proc/playsound, owner, 'massmeta/sound/magic/pass_create.ogg', 50, TRUE), 1)
		for(var/i in 1 to 2)
			var/obj/item/umbral_tendrils/T = new(owner, darkspawn)
			owner.put_in_hands(T)
	return TRUE
...
/datum/action/innate/darkspawn/pass/Deactivate()
	...
	playsound(owner, 'massmeta/sound/magic/pass_dispel.ogg', 50, 1)
